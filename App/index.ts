...
import Video from './server/src/schema/Video';
import KMP from './server/src/function/kmp_matching';
import path from 'path';
import 'dotenv/config';
import './server/src/db.ts';
import { signup, login } from './server/src/user';
import cors from 'cors';
import multer from 'multer';
import setUpload from './server/src/S3/upload';

const app = express();
const BASE_URL = __dirname.substr(0, __dirname.length);
...
const uploadVideo = async (req: any, res: any) => {
	...
	console.log(req.body);
	console.log('---');
	console.log(req.path); // /uploadVideo
...
videoRouter.route('/uploadVideo').post(setUpload('video-zensen6/video'), uploadVideo);
